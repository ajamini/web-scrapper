...
@app.get("/crawl_kijiji")
# Define a function to be executed when the endpoint is called.
# Add a description to the function.
def crawl_kijiji(postal: str, make: str, model: str):
    # Define the URL to scrape.
    url = f'https://www.kijijiautos.ca/cars/{make}/{model}'
    ...
    with sync_playwright() as p:
        ...
        time.sleep(2)
        try:
            page.wait_for_selector('div[class="l2GjMu"] button[class="hDZZVr bDZZVr b2MH0T f3yLIj"]').click()
            time.sleep(2)
            page.wait_for_selector('input[id="LocationAutosuggest"]').fill(postal)
            time.sleep(2)
            page.wait_for_selector('li[class="b3P4OF c3P4OF"]').click()
            time.sleep(2)
        ...
        parsed = []
        listings = soup.find_all('a', class_='bcNN7t')
        for listing in listings:
            try:
                # Get the item image.
                image = listing.find('img', class_='q1E1YI')['src']
                # Get the item title from span.
                title = listing.find('h2', 'G2jAym fcN7dZ z2jAym p2jAym b2jAym').text
                # Get the item price.
                price = listing.find('span', 'G2jAym d3uM7V C2jAym p2jAym b2jAym').text
                # Get the item URL.
                post_url = listing['href']
                # Get the item URL.
                attributes = listing.find('li', class_='c3n44x e3n44x')
                mileage = attributes.findNext('span').text
                # Get the item location.
                location = attributes.findNext('span').findNext('span').text
...
def crawl_facebook_marketplace(city: str, query: str, max_price: int):
    ...
    with sync_playwright() as p:
        ...
        time.sleep(2)
        try:
            email_input = page.wait_for_selector('input[name="email"]').fill('af_developer@yahoo.ca')
            password_input = page.wait_for_selector('input[name="pass"]').fill('Iasaiki*21')
            time.sleep(2)
        ...
        soup = BeautifulSoup(html, 'html.parser')
        parsed = []
        listings = soup.find_all('div', class_='x9f619 x1n2onr6 x1ja2u2z')
        for listing in listings:
            try:
                # Get the item image.
                ...
                # Get the item price.
                price = listing.find('span', 'x193iq5w xeuugli x13faqbe x1vvkbs xlh3980 xvmahel x1n0sxbx x1lliihq x1s928wv xhkezso x1gmr53x x1cpjm7i x1fgarty x1943h6x x4zkp8e x3x7a5m x1lkfr7t x1lbecb7 x1s688f xzsf02u').text
                # Get the item URL.
